...
name: computer-use
description: >-
  Use Legion's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, Legion app UI,
  or other desktop UI. Triggers include "computer use", "legion computer", "read
...
reference is served by the `legion` binary itself — kept out of this file on purpose so it can
never drift from the binary that will actually run your commands.

Engage Legion's computer-use surface whenever you must inspect or operate a local desktop app
...
windows, webviews, and Legion's own UI. Triggers include "computer use", "legion computer",
...
```text
LEGION skills get computer-use
```
...
```text
LEGION status --json
LEGION computer capabilities --json
LEGION computer list-apps --json
```

Then tell the user that updating Legion restores the full, version-matched guide via
`LEGION skills get computer-use`. Beyond these commands, ask the user rather than guessing a
